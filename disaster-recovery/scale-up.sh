...
if [[ "$database" == "foi-docreviewer" ]]; then

    echo "Turning on all consumers of the $database in the d7abee-$environment."

    oc project d7abee-$environment

    deployments_d7abee=$(oc get deployment -n d7abee-$environment -l docreviewer-disaster-recovery=true -o jsonpath='{.items[*].metadata.name}')

    echo "Found deployments. Starting to scale all deployments with label docreviewer-disaster-recovery=true to 1."
    ...
    echo "All deployments with label docreviewer-disaster-recovery=true have been scaled to one"

    echo "Turning on all consumers of the $database in the d106d6-$environment."

    oc project d106d6-$environment

    deployments_d106d6=$(oc get deployment -n d106d6-$environment -l docreviewer-disaster-recovery=true -o jsonpath='{.items[*].metadata.name}')

    echo "Found deployments. Starting to scale all deployments with docreviewer-disaster-recovery=true to zero."
    ...
    echo "All deployments with label docreviewer-disaster-recovery=true have been scaled to zero."
